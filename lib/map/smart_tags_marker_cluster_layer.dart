...
class SmartTagsMarkerClusterLayer extends StatefulWidget {
  final MarkerClusterLayerOptions options;
  final MapController mapController;
  final MapCamera mapCamera;

  const SmartTagsMarkerClusterLayer({
  ...
  @override
  State<SmartTagsMarkerClusterLayer> createState() => _SmartTagsMarkerClusterLayerState();
}

class _SmartTagsMarkerClusterLayerState extends State<SmartTagsMarkerClusterLayer> with TickerProviderStateMixin {
  ...
  _SmartTagsMarkerClusterLayerState();
  ...
  @override
  void didUpdateWidget(SmartTagsMarkerClusterLayer oldWidget) {
  ...
  VoidCallback _onClusterTap(MarkerClusterNode cluster) {
    return () async {
      ...
      // Honor synchronous camera updates from [onClusterTap] before zoom / spiderfy.
      final mapCamera = widget.mapController.camera;
      _currentZoom = mapCamera.zoom.ceil();
      ...
      final lacksSpread = _clusterBoundsLackGeographicSpread(cluster.bounds);
      ...
      final cannotDivide =
          ...
          cluster.markers.every(
            (marker) => marker.parent!.zoom == _maxZoom && marker.parent == cluster.markers.first.parent,
          ) ||
      ...
      final zoomTween = Tween<double>(begin: mapCamera.zoom, end: dest.zoom);
...
bool _clusterBoundsLackGeographicSpread(LatLngBounds bounds) {
  const epsilon = 1e-8;
