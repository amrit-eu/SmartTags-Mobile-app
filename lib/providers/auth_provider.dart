...
final authServiceProvider = Provider<AuthService>((ref) {
  return AuthService(authDao: ref.watch(databaseProvider).authDao);
});

/// A Riverpod [AsyncNotifier] managing authentication state.
///
/// Tracks the currently authenticated [User] or `null` if logged out.
...
class AuthNotifier extends AsyncNotifier<User?> {
  ...
  Future<User?> build() async {
    ...
    try {
      final result = await _callWithRefreshHandling<User?>(
  ...
  /// On success, sets state to [AsyncData] containing the [User].
